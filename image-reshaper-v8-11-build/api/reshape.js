...
Use only information visible in the attached source image.`;
  ...
  if(extraction||opts.allowModelFallback){
    ...
    if(text){
      console.warn("V8.11 source-analysis fallback to gpt-4.1 succeeded.");
      return text;
    }
  }
...
// V8.11 canonical source library: target dimensions never affect what exists in the source.
...
function applyProtectedFacts(composer,manifest){
  // V8.11: the layout model has zero authority to author display copy.
...
function augmentInstructionManifestFromRaw(raw="",manifest={}){
  ...
  // V8.11: newest user instruction is authoritative for deterministic text/styling.
...
if(req.method==="POST" && len>4_000_000){
  return res.status(413).json({error:"Upload is too large for the serverless function. V8.11 should compress the source image in the browser before upload; please refresh and try again."});
}
...
const legacyExact=((tr>2.4 && h<=160) || tr<0.2);
    // V8.11 keeps ONE canonical source manifest/planner, but extreme/shallow canvases
    ...
    if((UNIFIED_ENGINE && exactCanvasProfile) || (!UNIFIED_ENGINE && legacyExact)){
      ...
      if(requestMode==="edit_text" && priorPlan){
        ...
        return res.status(200).json({renderMode:'canvas-first-banner-composer',width:w,height:h,bannerPlan:edited,reuseAssets:true,
          sourceManifest:inventory,sourceElements:canonicalSourceElements(inventory,edited),validationSummary:'V8.11 EDIT_TEXT mode preserved the current banner layout and updated authoritative text only.'});
      }

      // V8.11 MODIFY MODE: update the LAST generated plan rather than starting over.
      if(requestMode==="replan" && priorPlan){
        ...
        return res.status(200).json({renderMode:'canvas-first-banner-composer',width:w,height:h,bannerPlan:replanned,reuseAssets:true,
          sourceManifest:inventory,sourceElements:canonicalSourceElements(inventory,replanned),validationSummary:'V8.11 re-planned the banner around authoritative text using the shared canonical source manifest.'});
      }
      ...
      if(requestMode==="regenerate" && priorPlan){
        ...
        return res.status(200).json({
          ...
          validationSummary:"V8.11 created an alternate banner layout and a new target-aware panoramic background from the cached canonical source manifest without re-running source analysis."
        });
      }
      ...
      if(requestMode==="modify" && priorPlan){
        ...
        // V8.11: `modified` already starts from priorPlan, so old locked subject values are inherited automatically.
...
return res.status(200).json({
          ...
          bannerPlan:modified,
          reuseAssets:true,
          validationSummary:"V8.11 modified the current design using a structured state patch. The newest user instruction was applied deterministically before layout, the current design state remained the baseline, and reusable source subject geometry/assets were preferred for resize or move requests when available."
        });
      }

      // V8.11 stage 0: understand Optional Instructions as a first-class manifest.
      ...
      // V8.11 stage 1: extract factual display copy before any layout/design reasoning.
      let protectedFacts;
      if(requestMode==="resize" && priorPlan?.sourceManifest){
      ...
      // V8.11: interpreted user corrections outrank extraction; regex parser remains as fallback.
      ...
      // V8.11: resolve natural-language "missing ending" headline requests against the ORIGINAL source.
      ...
      // V8.11: facts the user explicitly asks to see become protected display facts,
      ...
      // V8.11 stage 2: protected facts are re-applied AFTER planning. Planner metadata can never become drawable copy.
      ...
      // V8.11 stage 3: lock again after art direction and fail closed on any copy mutation/leak.
      composer=applyProtectedFacts(composer,protectedFacts);
...
V8.11 OPERATION MODES — APPLY TO EVERY DESTINATION SIZE:
...
// V8.11 OPERATION SEPARATION: EDIT_TEXT / MODIFY are current-design patches, never redesign prompts.
...
FAIL if IMAGE 2 looks like a new design/recomposition, moves unrelated groups, changes the background/layout substantially, or does not visibly implement the request.`;
  try{preservationVerdict=await visionCompare(key,baseline,candidate,comparePrompt,{maxOutput:700});}catch(e){console.warn('V8.11 preservation comparison unavailable:',e?.message||e);preservationVerdict='PASS: comparison unavailable; strict patch prompt used';}
  ...
  }catch(e){console.warn('V8.11 source QA unavailable:',e?.message||e);}
}
res.setHeader('Cache-Control','no-store');
let responsePlan={...universalPlan};
if((requestMode==='modify'||requestMode==='edit_text'||requestMode==='replan') && priorPlan){
  // V8.11: newest backend plan wins. Prior state fills gaps but must not erase a newly returned crop/scale/position.
