...
Use only information visible in the attached source image.`;
  ...
  if(extraction||opts.allowModelFallback){
    ...
    if(text){
      console.warn("V7.31 source-analysis fallback to gpt-4.1 succeeded.");
      return text;
    }
  }
...
async function edit(key,img,fn,type,prompt,size){let f=new FormData();f.append('model','gpt-image-1');f.append('image',new Blob([img],{type:type||'image/png'}),fn||'source.png');f.append('prompt',prompt);f.append('size',size);f.append('quality','high');f.append('output_format','png');f.append('input_fidelity','high');let r=await fetch('https://api.openai.com/v1/images/edits',{method:'POST',headers:{Authorization:`Bearer ${key}`},body:f}),d=await r.json();if(!r.ok)throw Error(d?.error?.message||'Image edit failed');let b=d?.data?.[0]?.b64_json;if(!b)throw Error('No image returned');return b}
...
function bannerPlanFromInventory(inventory,userInstructions=''){
  ...
  return {
    ...
    accentColor:/^#[0-9a-f]{6}$/i.test(accentRaw)?accentRaw:"#9ABB35",
...
function applyProtectedFacts(composer,manifest){
  // V7.31: the layout model has zero authority to author display copy.
...
function augmentInstructionManifestFromRaw(raw="",manifest={}){
  ...
  // V7.31: newest user instruction is authoritative for deterministic text/styling.
...
- MICRO_BANNER: prioritize recognizable headline/event identity + one key fact + real CTA when present. Do not cram address/secondary copy if it causes collision or tiny text. Main subject is supporting but recognizable. Zero overlaps between CTA and copy.
- COMPACT_BANNER: prioritize headline + date/time + subject; add CTA and venue/address when they remain clearly legible. Use a true horizontal composition, never a shrunken poster.
...
- User instructions: ${userInstructions||"none"}`;
  ...
  if(!/^#[0-9a-f]{6}$/i.test(accent))accent="#9ABB35";
  ...
  const actionCta=/(register|buy|book|learn|rsvp|reserve|tickets?|sign\s*up|join|donate|visit|apply|read|watch|listen|shop|get\s*tickets?)/i;
...
Accent color: ${composer.accent||"#9ABB35"}
...
- Preserve strong source-derived scene elements when they help the design, such as architecture, skyline, venue cues, lighting structures, gradients, environmental motifs, or recognizable background forms.
...
if(req.method==="POST" && len>4_000_000){
  return res.status(413).json({error:"Upload is too large for the serverless function. V7.31 should compress the source image in the browser before upload; please refresh and try again."});
}

if(req.method==='GET')return res.status(200).json({ok:true,route:'/api/reshape',version:'7.31'});
...
let inventory;
if((requestMode==="regenerate"||requestMode==="replan") && priorPlan?.sourceManifest){
...
if(requestMode==="replan" && priorPlan){
  const exactCurrent=[
    ...
    ...(Array.isArray(priorPlan.textElements)?priorPlan.textElements.map(x=>`${x.label||x.role||"Text"}: ${x.text}`):[])
  ].filter(Boolean).join(" | ");
  extra=`Re-plan the current composition around these exact authoritative text values. Preserve current visual assets. `+
    `Logos are immutable source assets and must not be redrawn, recolored, restyled, distorted, merged, simplified or altered. `+
    `${exactCurrent} ${extra||""}`.slice(0,3500);
}
...
let sp=spec(w,h),s=sp.safe,tr=w/h,formatClass=tr>5?'EXTREME_BANNER':tr>2.4?'WIDE_BANNER':tr<.45?'EXTREME_TALL':tr<.65?'TALL_NARROW':'STANDARD',useSafeCrop=false;
    if((tr>2.4 && h<=160) || tr<0.2){
      ...
      // V7.31 MODIFY MODE: update the LAST generated plan rather than starting over.
      if(requestMode==="replan" && priorPlan){
        const replanned={...priorPlan};
        const chars=(Array.isArray(priorPlan.textElements)?priorPlan.textElements:[]).filter(x=>x&&x.visible!==false)
          .reduce((n,x)=>n+String(x.text||'').length,0);
        const manuals=(Array.isArray(priorPlan.manualTextElements)?priorPlan.manualTextElements:[]).length;
        replanned.textZonePct=Math.min(72,Math.max(Number(priorPlan.textZonePct||48),48+manuals*7+Math.floor(chars/90)*4));
        replanned.subjectZonePct=Math.max(24,Math.min(Number(priorPlan.subjectZonePct||42),100-replanned.textZonePct-8));
        replanned.subjectX=replanned.textZonePct>58?.78:Number(priorPlan.subjectX||.68);
        replanned.subjectAnchor=replanned.subjectX>.6?'right-third':(replanned.subjectAnchor||'center');
        replanned.subjectPositionLocked=true;
        replanned.manualTextPlacement='layout';replanned.logosImmutable=true;
        replanned.sourceManifest=priorPlan.sourceManifest||inventory;
        replanned.textElements=Array.isArray(priorPlan.textElements)?priorPlan.textElements:[];
        replanned.manualTextElements=Array.isArray(priorPlan.manualTextElements)?priorPlan.manualTextElements:[];
        replanned.textStyles=priorPlan.textStyles||{};replanned.userTextLocked=true;
        ...
        return res.status(200).json({renderMode:"canvas-first-banner-composer",width:w,height:h,bannerPlan:replanned,reuseAssets:true,
          sourceManifest:inventory,validationSummary:"V7.31 re-planned the current banner around authoritative text without re-running source analysis or altering logos."});
      }
      ...
      if(requestMode==="regenerate" && priorPlan){
        ...
        return res.status(200).json({
          ...
          validationSummary:"V7.31 created an alternate banner layout from the cached design/source analysis without re-running source analysis."
        });
      }
      ...
      if(requestMode==="modify" && priorPlan){
        ...
        // V7.31: the browser's deterministic visual state is authoritative too.
...
return res.status(200).json({
          ...
          bannerPlan:modified,
          reuseAssets:true,
          validationSummary:"V7.31 modified the current design using a structured state patch. The newest user instruction was applied deterministically before layout, and current visual assets were preserved."
        });
      }

      // V7.31 stage 0: understand Optional Instructions as a first-class manifest.
      ...
      // V7.31 stage 1: extract factual display copy before any layout/design reasoning.
      let protectedFacts;
      ...
      // V7.31: interpreted user corrections outrank extraction; regex parser remains as fallback.
      ...
      // V7.31: resolve natural-language "missing ending" headline requests against the ORIGINAL source.
      ...
      // V7.31: facts the user explicitly asks to see become protected display facts,
      ...
      // V7.31 stage 2: protected facts are re-applied AFTER planning. Planner metadata can never become drawable copy.
      ...
      composer.canvasClass=(h<=55&&tr>4.5)?"MICRO_BANNER":
        (h<=120&&tr>2.4)?"COMPACT_BANNER":
        (tr>5)?"LEADERBOARD":
        (tr>2.4)?"WIDE_BANNER":
      ...
      // V7.31 stage 3: lock again after art direction and fail closed on any copy mutation/leak.
      composer=applyProtectedFacts(composer,protectedFacts);
...
// For non-canvas modify flows, edit the CURRENT rendered design, not the original upload.
