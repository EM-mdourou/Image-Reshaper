...
Use only information visible in the attached source image.`;
  ...
  if(extraction||opts.allowModelFallback){
    ...
    if(text){
      console.warn("V7.28 source-analysis fallback to gpt-4.1 succeeded.");
      return text;
    }
  }
...
function applyProtectedFacts(composer,manifest){
  // V7.28: the layout model has zero authority to author display copy.
...
function augmentInstructionManifestFromRaw(raw="",manifest={}){
  ...
  // V7.28: newest user instruction is authoritative for deterministic text/styling.
...
if(req.method==="POST" && len>4_000_000){
  return res.status(413).json({error:"Upload is too large for the serverless function. V7.28 should compress the source image in the browser before upload; please refresh and try again."});
}

if(req.method==='GET')return res.status(200).json({ok:true,route:'/api/reshape',version:'7.28'});
...
For banner output, visual interest is required: use a focused source visual and source-derived color whenever possible.`,{mode:"extract",effort:"none",recoveryEffort:"none",maxOutput:3600,recoveryMaxOutput:4200,allowModelFallback:true});let sp=spec(w,h),s=sp.safe,tr=w/h,formatClass=tr>5?'EXTREME_BANNER':tr>2.4?'WIDE_BANNER':tr<.45?'EXTREME_TALL':tr<.65?'TALL_NARROW':'STANDARD',useSafeCrop=false;
    if((tr>2.4 && h<=160) || tr<0.2){
      ...
      // V7.28 MODIFY MODE: update the LAST generated plan rather than starting over.
      if(requestMode==="modify" && priorPlan){
        ...
        // V7.28: the browser's deterministic visual state is authoritative too.
...
return res.status(200).json({
          ...
          bannerPlan:modified,
          reuseAssets:true,
          validationSummary:"V7.28 modified the current design using a structured state patch. The newest user instruction was applied deterministically before layout, and current visual assets were preserved."
        });
      }

      // V7.28 stage 0: understand Optional Instructions as a first-class manifest.
      ...
      // V7.28 stage 1: extract factual display copy before any layout/design reasoning.
      let protectedFacts;
      ...
      // V7.28: interpreted user corrections outrank extraction; regex parser remains as fallback.
      ...
      // V7.28: resolve natural-language "missing ending" headline requests against the ORIGINAL source.
      ...
      // V7.28: facts the user explicitly asks to see become protected display facts,
      ...
      // V7.28 stage 2: protected facts are re-applied AFTER planning. Planner metadata can never become drawable copy.
      ...
      // V7.28 stage 3: lock again after art direction and fail closed on any copy mutation/leak.
      composer=applyProtectedFacts(composer,protectedFacts);
