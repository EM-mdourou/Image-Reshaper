...
global.fetch=async(url,opts={})=>{
  ...
  if(String(url).includes('/v1/responses')){
    ...
    try{
      ...
      if(/SOURCE FACT MANIFEST/i.test(prompt) && /MANIFEST_HEADLINE/i.test(prompt)) text=manifest;
...
function multipart(fields){
  const boundary='----v81testboundary';
  const chunks=[];
...

{
  ...
  assert.equal(r.json.exportMode,'safe-crop');
  assert.ok(Math.abs((r.json.safeCrop.w/r.json.safeCrop.h)-(880/460))<0.005);
}
...
for(const mode of ['edit_text','modify','regenerate']){
  ...
  assert.equal(r.json.renderMode,'unified-ai-composer');
  assert.equal(r.json.exportMode,'safe-crop');
  assert.ok(r.json.safeCrop && r.json.safeCrop.w>0 && r.json.safeCrop.h>0);
  const ratio=r.json.safeCrop.w/r.json.safeCrop.h;
  assert.ok(Math.abs(ratio-(880/460))<0.005,`safe crop ratio for ${mode}`);
}

console.log('PASS v8.1 mocked handler operation tests');
